...
\begin{enumerate}
\item Writing Haskell-like Agda (no need to cover the whole source language)
...
\item Ensure all dependent types appear under \textit{erased} positions
  ...
  \end{itemize}
\item Translation of each language construct has equivalent behaviour
  \begin{itemize}
...
\item Monadic code
\onslide<2->{\alert{$\sim$ still need to reverse desugaring}}
\item Infinite data
\onslide<2->{\alert{$\sim$ coinductive types}}
\item Non-termination, general recursion
\onslide<2->{\alert{$\sim$ \alert{partiality}/general monad}}
\end{itemize}
\onslide<3->{
Extra goodies:
\begin{itemize}
