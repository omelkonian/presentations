...
open import Ledger.TokenAlgebra PolicyId
  using (TokenAlgebra; CommMonoid; commMonoid)
...
\begin{code}
  ...
  TokenMap = AssetId ⇒ Quantity
  instance
    Semigroup-TokenMap : Semigroup TokenMap
    Semigroup-TokenMap ._◇_ = _⊕_

    Monoid-TokenMap : Monoid TokenMap
    Monoid-TokenMap .ε = ι

    CommMonoid-TokenMap : CommMonoid TokenMap _≋_
    CommMonoid-TokenMap = record
      { isCommMonoid = record {isMonoid = ≋-⊕-ι-isMonoid; comm = ⊕-comm} }
  ...
  Value-TokenAlgebra specialPolicy specialAsset size = record
    ...
    open CommutativeMonoid commMonoid             using () renaming (rawMonoid to Vcm-mon)
