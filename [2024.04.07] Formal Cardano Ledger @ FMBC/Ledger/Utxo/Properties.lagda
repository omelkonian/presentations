...
balance-cong : proj₁ utxo ≡ᵉ proj₁ utxo' → balance utxo ≈ᶜᵐ balance utxo'
...
balance-cong-coin {utxo} {utxo'} x =
  coinHomo .⟦⟧-cong (balance-cong {utxo} {utxo'} x)
  where open MonoidMorphisms.IsMonoidHomomorphism

open MonoidMorphisms.IsMonoidHomomorphism
private
  ∙-homo-Coin = IsMagmaHomomorphism.homo (isMagmaHomomorphism coinHomo)
...
module _ {txb : _} (open TxBody txb) where
  ...
  consumedCoinEquality {utxoState} {pp} h = let utxo = UTxOState.utxo utxoState in
    ...
    coin (balance (utxo ∣ txins) + mint) + coin (inject $ depositRefunds pp utxoState txb)
      ≡⟨ cong (coin (balance (utxo ∣ txins) + mint) +_) (coin∘inject _) ⟩
...
module DepositHelpers
  ...
  {Γ : UTxOEnv}
  (step : Γ ⊢ ⟦ utxo  , deposits  , fees  , donations ⟧ᵘ ⇀⦇ tx ,UTXO⦈
              ⟦ utxo' , deposits' , fees' , donations' ⟧ᵘ)
  ...
  private
    ...
    utxoSt : UTxOState
    utxoSt = ⟦ utxo , deposits , fees , donations ⟧ᵘ
    ref tot : Coin
    ...
    newBal' : Γ ⊢ ⟦ utxo  , deposits  , fees  , donations ⟧ᵘ ⇀⦇ tx ,UTXO⦈
                  ⟦ utxo' , deposits' , fees' , donations' ⟧ᵘ
            → consumed pp utxoSt txb ≡ produced pp utxoSt txb
    newBal' (UTXO-inductive⋯ _ _ _ _ _ _ _ _ x _ _ _ _ _ _) = x
    newBal : consumed pp utxoSt txb ≡ produced pp utxoSt txb
    newBal = newBal' step
    noMintAda' : Γ ⊢ ⟦ utxo  , deposits  , fees  , donations ⟧ᵘ ⇀⦇ tx ,UTXO⦈
                     ⟦ utxo' , deposits' , fees' , donations' ⟧ᵘ
               → coin (mint) ≡ 0
    noMintAda' (UTXO-inductive⋯ _ _ _ _ _ _ _ _ _ x _ _ _ _ _) = x
    noMintAda : coin mint ≡ 0
    noMintAda = noMintAda' step
    remDepTot : Coin
    remDepTot = getCoin deposits - ref

  uDep≡ : Γ ⊢ ⟦ utxo  , deposits  , fees  , donations ⟧ᵘ ⇀⦇ tx ,UTXO⦈
              ⟦ utxo' , deposits' , fees' , donations' ⟧ᵘ
...
\label{prop:pov}
\noindent
Let \getCoin be the sum of all coins contained within a \UTxOState.
Then, for all $\AgdaBound{Γ}\in$ \UTxOEnv, $\AgdaBound{s},
\AgdaBound{s'}\in$ \UTxOState and $\var{tx}\in$ \Tx,
if
\begin{code}[hide]
pov : let open Tx; open TxBody in ∀ {s s'} →
\end{code}
\begin{code}[inline]
  tx .body .txid ∉ mapˢ proj₁ (dom (s .UTxOState.utxo))
\end{code}
and
\begin{code}[hide]
  →
...
\begin{code}[hide]
pov {tx = tx}
    {s = utxoSt@(⟦ utxo , deposits , fees , donations ⟧ᵘ)}
    {s' = ⟦ _ , deposits' , _ , _ ⟧ᵘ}
    h' step@(UTXO-inductive⋯ _ Γ _ _ _ _ _ newBal noMintAda _ _ _ _ _ _) =
