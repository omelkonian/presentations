...
∈-removeAt⁺ : ∀ (y∈ : y ∈ xs) →
  ...
  ∙ x ∈ xs
    ──────────────────────────────────
    x ∈ L.removeAt xs (L.Any.index y∈)
∈-removeAt⁺ (here refl) x≢ (here x≡)   = ⊥-elim $ x≢ x≡
∈-removeAt⁺ (here refl) x≢ (there x∈)  = x∈
∈-removeAt⁺ (there y∈)  x≢ (here refl) = here refl
∈-removeAt⁺ (there y∈)  x≢ (there x∈)  = there (∈-removeAt⁺ y∈ x≢ x∈)
...
reverse-∷ʳ : ∀ {A : Type ℓ} (x : A) (xs : List A) →
  L.reverse (xs L.∷ʳ x) ≡ x ∷ L.reverse xs
reverse-∷ʳ _ [] = refl
reverse-∷ʳ x (y ∷ xs) =
  begin
    L.reverse ((y ∷ xs) L.∷ʳ x)
  ≡⟨⟩
    L.reverse (y ∷ (xs L.∷ʳ x))
  ≡⟨ L.unfold-reverse y (xs L.∷ʳ x) ⟩
    L.reverse (xs L.∷ʳ x) L.∷ʳ y
  ≡⟨ cong (L._∷ʳ y) $  reverse-∷ʳ x xs ⟩
    x ∷ (L.reverse xs L.∷ʳ y)
  ≡˘⟨ cong (x ∷_) $ L.unfold-reverse y xs ⟩
    x ∷ L.reverse (y ∷ xs)
...
data ProperSuffix {A : Type} : List A → List A → Type where
  there : Suffix≡ xs ys → ProperSuffix xs (x ∷ ys)

wellFoundedProperSuffix : (xs : List A) → Acc ProperSuffix xs
wellFoundedProperSuffix []       = acc λ ()
wellFoundedProperSuffix (x ∷ xs) = acc λ where
    (there ys≤xs) → go ys≤xs
  where
    go : ∀ {xs ys} → (ys≤xs : Suffix≡ ys xs) → Acc ProperSuffix ys
    go (here ys≡xs) with refl ← L.PW.Pointwise-≡⇒≡ ys≡xs
                     = wellFoundedProperSuffix _
    go (there ys≤xs) = go ys≤xs

ProperSuffix-cons : ProperSuffix xs (x ∷ xs)
ProperSuffix-cons = there (≡⇒Suffix refl)
